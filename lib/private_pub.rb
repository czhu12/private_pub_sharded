...
module PrivatePub
  ...
  class << self
    ...
    def subscription(options = {})
      #sub = {:server => get_url_string(options[:channel]), :timestamp => (Time.now.to_f * 1000).round}.merge(options)
      sub = {:server => "http://localhost:9292/faye", :timestamp => (Time.now.to_f * 1000).round}.merge(options)
